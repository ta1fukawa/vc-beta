...
model = XvecModel(64).to('cuda')
model.load_state_dict(torch.load(sorted(glob.glob('dest/default_xvec/stretch/*/weights.pth'))[-1]))
model.eval()

pred_list = list()
for data in loader:
    with torch.no_grad():
        pred = model.embed(data)
...
np.savez_compressed('resource/emb/xvec_centroids.npz', centroids=centroids)
